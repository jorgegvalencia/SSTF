...
if (isset($_SESSION['PHPSESSID']) && isset($_SESSION['USER_ID'])){
	...
	$result = mysqli_query($conn,"SELECT A.* , F.`posicion` FROM Usuario_has_Favorito as F, Accion as A WHERE F.`Usuario_idUsuario`='".$_SESSION['USER_ID']."' and F.`Accion_codigo`=A.`codigo` ORDER BY F.posicion ASC");
	if(!$result){
		print_r($conn->error);
